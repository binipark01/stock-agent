...
class TelegramNotifyTest(unittest.TestCase):
    ...
    def test_load_config_prefers_us_specific_chat_and_thread(self):
        from src.telegram_notify import load_telegram_config

        env = {
            "TELEGRAM_BOT_TOKEN": "token",
            "TELEGRAM_CHAT_ID": "generic-room",
            "TELEGRAM_THREAD_ID": "100",
            "TELEGRAM_US_CHAT_ID": "us-room",
            "TELEGRAM_US_THREAD_ID": "200",
            "TELEGRAM_KR_CHAT_ID": "kr-room",
            "TELEGRAM_KR_THREAD_ID": "300",
        }

        with patch.dict(os.environ, env, clear=True):
            config = load_telegram_config()

        self.assertEqual(config.chat_id, "us-room")
        self.assertEqual(config.thread_id, "200")

    def test_load_config_can_select_kr_chat_with_scope_override(self):
        from src.telegram_notify import load_telegram_config
        ...
        with patch.dict(os.environ, env, clear=True):
            config = load_telegram_config("KR")

        self.assertEqual(config.chat_id, "kr-room")

    def test_load_config_prefers_scoped_bot_token(self):
        from src.telegram_notify import load_telegram_config

        env = {
            "TELEGRAM_BOT_TOKEN": "generic-token",
            "TELEGRAM_US_BOT_TOKEN": "us-token",
        ...
        self.assertEqual(config.bot_token, "us-token")
