...
class SectorStrengthAlertRunnerTest(unittest.TestCase):
    ...
    def test_build_alert_text_uses_readable_six_part_telegram_template(self) -> None:
        ...
        for heading in ("1) 시장", "2) 강한 테마", "3) 약한 테마", "4) 먼저 볼 종목", "5) 결론", "• 결론:"):
        ...
        self.assertIn("스토캐스틱 Slow 88/82(+4): 과열권 K>D 유지", text)
        self.assertNotIn("구름", text)
        self.assertNotIn("전환선", text)
        self.assertNotIn("기준선", text)
        self.assertIn("BB 92%(+8): 상단 확장", text)
    ...
    def test_build_alert_text_formats_long_movers_as_mobile_friendly_bullets(self) -> None:
        ...
        self.assertIn("\n  · 스토캐스틱 Slow 88/82(+4): 과열권 K>D 유지", text)
        self.assertNotIn("구름", text)
        self.assertNotIn("전환선", text)
        self.assertNotIn("기준선", text)
        self.assertIn("\n  · BB 92%(+8):", text)
        self.assertIn("\n  · 종합: 추세·모멘텀 개선 중", text)
        self.assertIn("\n• 로테이션: 우주 내부 발사체로 자금 이동", text)
        self.assertNotIn("DXY", text)
        self.assertNotIn("금리", text)
        self.assertLessEqual(len(text), SECTOR_ALERT_COMPACT_LIMIT)
    ...
    def test_build_alert_text_keeps_movers_and_omits_etf_reference_when_rotation_lines_expand_focus(self) -> None:
        from scripts.run_sector_strength_alerts import build_alert_text
        ...
        self.assertIn("• 로테이션: 반도체/AI칩 내부 메모리/스토리지로 자금 이동", text)
        self.assertIn("• 로테이션: AI 가속기/GPU 약세", text)
        self.assertNotIn("\n• 반도체\n• AI칩", text)
        self.assertNotIn("QQQ", text)
        self.assertNotIn("[truncated]", text)
        self.assertLessEqual(len(text), SECTOR_ALERT_COMPACT_LIMIT)
    ...
    def test_build_alert_text_omits_intraday_risk_spikes_from_telegram_body(self) -> None:
        ...
        self.assertIn("5분 뒤 SPY·주도테마 재확인", text)
    ...
    def test_build_alert_text_compacts_verbose_theme_leaders_without_cutting_sections(self) -> None:
        ...
        for heading in ("4) 먼저 볼 종목", "5) 결론", "• 결론:"):
            self.assertIn(heading, text)
    ...
    def test_build_alert_text_strips_volume_day_over_day_delta(self) -> None:
        from scripts.run_sector_strength_alerts import build_alert_text
        ...
        text = build_alert_text({
            ...
            "focus": [
                "장 분위기: 중립 / VIX 충격 없음",
                "강한 테마: 우주/항공우주 평균 +2.00% / 상승비율 80.0% / 거래대금 7.2M / 거래량 당일 1.5M / 전일 3.0M / 전일대비 -50.00% / 주도 RKLB +4.00%",
                "약한 테마: 양자/차세대컴퓨팅 평균 -2.00% / 상승비율 20.0% / 거래대금 2.0M / 거래량 당일 500.0K / 전일 1.0M / 전일대비 -50.00% / 주도 IONQ -1.00%",
                "테마별 대장주: 우주/항공우주: RKLB +4.00% 가격 30 / 거래량 당일 1.0M / 전일 2.0M / 전일대비 -50.00%, 정규장, 주도 — RSI 64(+5): 50선 위에서 재가속; MACD 0.72/0.41 h+0.31(+0.09): 신호선 위·히스토그램 확대; 스토캐스틱 Slow 88/82(+4): 과열권 K>D 유지; BB 92%(+8) 상단권: 상단 확장",
                "전일종가 대비 현재 강세: RKLB +10.00% 가격 30 / 거래량 당일 1.0M / 전일 2.0M / 전일대비 -50.00% / 기준 전일 정규장 종가 대비 현재가 / 출처 Yahoo chart 1m includePrePost",
                "장 분위기: NASDAQ +0.10% / SPY +0.00% / SOXX +0.20% / BTCUSDT +0.30% / WTI -0.10% / VIX -0.20% / 기준시각 2026-05-07T13:35:00+00:00",
            ],
            "next_actions": ["조건은 VWAP 눌림 대기"],
        })

        self.assertIn("거래량 당일 1.5M / 전일 3.0M", text)
        self.assertIn("거래량 당일 1.0M / 전일 2.0M", text)
        self.assertNotIn("전일대비", text)
        self.assertNotIn("- 참고:", text)
        self.assertIn("4) 먼저 볼 종목", text)
        self.assertIn("현재 강세", text)
        self.assertNotIn("전일종가 대비 현재 강세", text)
    ...
    def test_build_alert_text_surfaces_flow_proxy_line(self) -> None:
        ...
        self.assertNotIn("SPY 대비", text)
        self.assertIn("BB 85%(+5)", text)
        self.assertNotIn("BB 85%(+5) 상단근접", text)
        self.assertNotIn("상단근접", text)
        self.assertIn("5) 결론", text)
        self.assertIn("단정 금지", text)

    def test_build_alert_text_keeps_up_to_five_strong_themes(self) -> None:
        from scripts.run_sector_strength_alerts import build_alert_text

        strong_items = [
            f"테마{i} 평균 +{i}.00% / 상승비율 8{i}.0% / 거래대금 .0B / 주도 AAA{i} +{i}.00%"
            for i in range(1, 7)
        ]
        text = build_alert_text({
            "mode": "sector_strength",
            "summary": "장중 테마 강약: 테마1 주도 / 테마6 약세 / 시장 리스크오프",
            "collected_at": "2026-04-30T13:35:00+00:00",
            "focus": [
                "장 분위기: 중립 / SPY 보합",
                "강한 테마: " + " | ".join(strong_items),
                "약한 테마: 방어 평균 -0.20% / 상승비율 40.0% / 주도 DEF -1.00%",
            ],
            "next_actions": [],
        })

        for i in range(1, 6):
            self.assertIn(f"테마{i} 평균", text)
        self.assertNotIn("테마6 평균", text)
        self.assertNotIn("시장 리스크오프", text)

    def test_build_alert_text_labels_positive_weak_theme_as_relative_weakness(self) -> None:
        from scripts.run_sector_strength_alerts import build_alert_text

        text = build_alert_text({
            "mode": "sector_strength",
            "summary": "장중 테마 강약: 양자/차세대컴퓨팅 주도 / 헬스케어/GLP-1/디지털헬스 약세",
            "collected_at": "2026-04-30T13:35:00+00:00",
            "focus": [
                "강한 테마: 양자/차세대컴퓨팅 평균 +3.00% / 상승비율 88.0% / 거래대금 .0B / 주도 QBTS +4.00%",
                "약한 테마: 헬스케어/GLP-1/디지털헬스 평균 +0.43% / 상승비율 62.5% / 거래대금 .7B / 주도 LLY +1.90%",
            ],
            "next_actions": [],
        })

        self.assertIn("헬스케어/GLP-1/디지털헬스 상대 약세", text)
        self.assertNotIn("헬스케어/GLP-1/디지털헬스 약세", text)

    def test_build_alert_text_keeps_negative_low_breadth_theme_as_weakness(self) -> None:
        from scripts.run_sector_strength_alerts import build_alert_text

        text = build_alert_text({
            "mode": "sector_strength",
            "summary": "장중 테마 강약: 양자/차세대컴퓨팅 주도 / 암호화/코인 관련주 약세",
            "collected_at": "2026-04-30T13:35:00+00:00",
            "focus": [
                "강한 테마: 양자/차세대컴퓨팅 평균 +3.00% / 상승비율 88.0% / 거래대금 .0B / 주도 QBTS +4.00%",
                "약한 테마: 암호화/코인 관련주 평균 -0.18% / 상승비율 41.2% / 거래대금 .7B / 주도 CLSK +3.40%",
            ],
            "next_actions": [],
        })

        self.assertIn("암호화/코인 관련주 약세", text)
        self.assertNotIn("암호화/코인 관련주 상대 약세", text)
        self.assertLessEqual(len(text), SECTOR_ALERT_COMPACT_LIMIT)
