...
DEFAULT_TELEGRAM_MARKET_SCOPE = "US"
...
def _market_scope(file_values: dict[str, str], explicit_scope: str | None = None) -> str:
    raw = explicit_scope or _env_value("TELEGRAM_MARKET_SCOPE", file_values) or DEFAULT_TELEGRAM_MARKET_SCOPE
    return _normalize_market_scope(raw)


def _scoped_env_value(base_key: str, file_values: dict[str, str], scope: str, default: str = "") -> str:
    if scope:
        scoped = _env_value(f"TELEGRAM_{scope}_{base_key}", file_values).strip()
        if scoped:
            return scoped
    return _env_value(f"TELEGRAM_{base_key}", file_values, default)
    ...
    try:
        conn.request(
            ...
            headers={
                ...
                "Connection": "close",
                "User-Agent": "us-stock-agent-telegram-notify",
            },
        )
...
def load_telegram_config(market_scope: str | None = None) -> TelegramConfig:
    env_file = os.getenv("TELEGRAM_ENV_FILE")
    file_values = _read_env_file(env_file)
    scope = _market_scope(file_values, market_scope)
    dry_run = _truthy(_env_value("TELEGRAM_NOTIFY_DRY_RUN", file_values))
    token = _scoped_env_value("BOT_TOKEN", file_values, scope).strip()
    chat_id = _scoped_env_value("CHAT_ID", file_values, scope).strip()
    if not chat_id:
        allowed_users = _scoped_env_value("ALLOWED_USERS", file_values, scope).strip()
        chat_id = next((part.strip() for part in allowed_users.replace(";", ",").split(",") if part.strip()), "")
    thread_id = _scoped_env_value("THREAD_ID", file_values, scope).strip() or None
