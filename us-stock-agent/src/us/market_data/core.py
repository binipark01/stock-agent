...
def fetch_price_ohlcv_history(symbol: str, range_period: str = "1y", interval: str = "1d") -> list[dict[str, float]]:
    quote_symbol = quote(symbol)
    url = f"https://query1.finance.yahoo.com/v8/finance/chart/{quote_symbol}?range={range_period}&interval={interval}"
    payload = _fetch_json(url)
    if payload:
        ...
        for idx, close in enumerate(closes):
            ...
            try:
                ...
                continue
            def value_at(items: list[Any], fallback: float) -> float:
                try:
                    value = items[idx]
            ...
            records.append(
                {
                    "open": value_at(opens, close_float),
                    "high": value_at(highs, close_float),
                    "low": value_at(lows, close_float),
                    "close": close_float,
                    "volume": value_at(volumes, 0.0),
                }
            )
        if len(records) >= 30:
            return records

    return fetch_price_ohlcv_history_yfinance(symbol, range_period=range_period, interval=interval)
...
def fetch_price_ohlcv_history_yfinance(symbol: str, range_period: str = "1y", interval: str = "1d") -> list[dict[str, float]]:
    """Optional yfinance OHLCV fallback for technical snapshots."""
    try:
        import yfinance as yf  # type: ignore
    except Exception:
        return []
    ...
    try:
        history = yf.Ticker(symbol).history(period=range_period, interval=interval, auto_adjust=False)
        records: list[dict[str, float]] = []
        for _, row in history.dropna(subset=["Close"]).iterrows():
