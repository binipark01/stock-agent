...
def _aggregate_volume_fields(rows: list[dict[str, Any]]) -> dict[str, Any]:
    day_values = [_first_numeric(row.get("day_volume"), row.get("volume")) for row in rows]
...
def _volume_inline_text(row: dict[str, Any]) -> str:
    day_volume = _first_numeric(row.get("day_volume"), row.get("volume"))
    ...
    if day_volume is not None:
        parts.append(f"당일 {_fmt_volume(day_volume)}")
    if previous_volume is not None:
        parts.append(f"전일 {_fmt_volume(previous_volume)}")
    volume_vs_previous_pct = _to_float(row.get("volume_vs_previous_pct"))
    if volume_vs_previous_pct is not None:
        parts.append(f"전일대비 {_fmt_pct(volume_vs_previous_pct)}")
    return " / 거래량 " + " / ".join(parts)
...
def _theme_line_for(prefix: str, rows: list[dict[str, Any]]) -> str:
    ...
    for row in rows[:3]:
        ...
        volume_text = _volume_inline_text(row)
        parts.append(
            f"{row['name']} 평균 {_fmt_pct(row['average_pct_change'])} / 상승비율 {row['breadth_positive_pct']:.1f}%{value_text}{volume_text} / 주도 {leaders or 'n/a'}"
        )
    return f"{prefix}: " + " | ".join(parts)
