...
DEFAULT_THREADS_SEED_PATH = REPO_ROOT / "config" / "threads_seed_accounts.json"
DEFAULT_THREADS_CLASSIFIED_PATH = REPO_ROOT / "config" / "threads_seed_accounts_classified.json"
DEFAULT_THREADS_VIEW_TARGETS_PATH = REPO_ROOT / "config" / "threads_view_targets.json"


def load_threads_view_targets(path: str | Path | None = None) -> list[dict[str, Any]]:
    target_path = Path(path or DEFAULT_THREADS_VIEW_TARGETS_PATH)
    ...
    for item in targets:
        ...
        if not handle:
            continue
        normalized.append({**item, "handle": handle, "display_name": display_name})
    return normalized
